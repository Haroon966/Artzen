...
async function main() {
  ...
  const placeholderCount = products.filter((p) => p.image === PLACEHOLDER_IMAGE).length;
  if (products.length > 0 && placeholderCount / products.length > 0.5) {
    throw new Error(
      `Aborting: ${placeholderCount}/${products.length} products have no downloaded image. ` +
        "WooCommerce image fetch likely failed — refusing to overwrite catalog with placeholders."
    );
  }
