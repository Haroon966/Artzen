...
import { formatPrice } from "@/lib/checkout-delivery";
import {
...
export function CartContent() {
  ...
  const handleCheckout = async () => {
    ...
    if (!shopifyEnabled) {
      const msg =
        "Shopify checkout is not configured. Set NEXT_PUBLIC_SHOPIFY_STORE_DOMAIN (Basic plan cart redirect — e.g. store.artzens.com).";
      setLocalError(msg);
      trackCheckoutSubmitFailed("shopify_not_configured");
      return;
    }
    ...
    try {
      ...
      if (!url) {
        const msg =
          cartError ||
          "Could not open Shopify checkout. Products need Shopify variant ids (import + sync catalog).";
        setLocalError(msg);
        trackCheckoutSubmitFailed("missing_checkout_url");
        return;
      }
      // Standard Basic-plan redirect: Shopify hosted cart/checkout
      window.location.assign(url);
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : "Checkout failed unexpectedly";
      setLocalError(msg);
      trackCheckoutSubmitFailed("checkout_exception");
    } finally {
  ...
  const busy = cartBusy || checkoutPending;
  const errorMessage = localError || cartError;
              ...
              <div className="flex min-w-0 flex-1 flex-col justify-center gap-3 px-5 py-5 sm:px-6 sm:py-5">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <button
                    ...
                    className="font-[var(--font-dm-sans)] text-[12px] font-medium uppercase tracking-wider text-[var(--muted)] underline decoration-black/20 underline-offset-4 transition hover:text-[#8b2525] hover:decoration-[#8b2525]/40 disabled:opacity-50"
                  >
                    Remove
                  </button>
                </div>
        ...
        >
          ...
          <div className="mt-6 space-y-3 border-b border-white/10 pb-6 font-[var(--font-dm-sans)] text-[14px]">
            ...
            <div className="flex justify-between text-cream-deep/65">
              <span>Delivery</span>
              <span className="text-[var(--gold2)]">Calculated at checkout</span>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-lg border border-white/10 bg-white/5 p-4">
            <p className="font-[var(--font-dm-sans)] text-[11px] font-medium uppercase tracking-[0.08em] text-cream-deep/70">
              Shopify checkout
            </p>
            <p className="mt-2 font-[var(--font-dm-sans)] text-[12px] leading-relaxed text-cream-deep/75">
              You&apos;ll finish address and payment on Shopify (works on Basic
              plan). Cash on Delivery appears when enabled in Shopify Admin.
            </p>
          </div>
          {errorMessage ? (
            <p
              role="alert"
              className="mt-4 font-[var(--font-dm-sans)] text-[12px] leading-relaxed text-[#f0a0a0]"
            >
              {errorMessage}
            </p>
          ) : null}
          <button
            ...
            disabled={busy}
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-[var(--gold)] py-3.5 font-[var(--font-dm-sans)] text-[14px] font-semibold text-coffee-bean transition hover:bg-[var(--gold2)] hover:shadow-lg hover:shadow-[var(--gold)]/20 disabled:opacity-60"
          >
          ...
          <p className="mt-6 text-center font-[var(--font-dm-sans)] text-[12px] font-light leading-relaxed text-cream-deep/70">
            COD total is confirmed at checkout before dispatch.
          </p>
        </motion.div>
      </aside>
    </div>
