...
import { CustomerDetailsFields } from "@/components/CustomerDetailsFields";
...
export function CartContent() {
  ...
  const handleWhatsAppOrder = () => {
    ...
    const message = buildFullOrderWhatsAppMessage({
      formData,
      items,
      totalPrice,
      orderRef,
    });
    saveCheckoutDeliveryToLocalStorage({ ...formData });
    ...
    void saveOrderToHistory({
      ...
      // Non-blocking: order must proceed even if IndexedDB is unavailable
    });

    trackCheckoutSubmitAttempt(items, totalPrice);
    trackPurchase(orderRef, items, totalPrice);
    trackWhatsAppOpenAttempt("cart");
        ...
        >
          ...
          <div className="mt-6 rounded-lg border border-white/10 bg-white/5 p-4">
            ...
            <p className="mt-2 font-[var(--font-dm-sans)] text-[12px] leading-relaxed text-cream-deep/75">
              Enter your details below — we&apos;ll pre-fill your WhatsApp message with your order
              and address.
            </p>
            <div className="mt-4">
            ...
            <p className="mt-3 font-[var(--font-dm-sans)] text-[12px] text-cream-deep/75">
              Estimated delivery fee:{" "}
              <span className="font-medium text-cream-soft">
                {cityEstimateLabel(formData.city)}
              </span>
            </p>
          </div>
          <button
            ...
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] py-3.5 font-[var(--font-dm-sans)] text-[14px] font-semibold text-white transition hover:bg-[#20bd5a] hover:shadow-lg hover:shadow-[#25D366]/20"
          >
          ...
          <p className="mt-6 text-center font-[var(--font-dm-sans)] text-[12px] font-light leading-relaxed text-cream-deep/70">
            Fill in your delivery details, then tap Order on WhatsApp — we confirm delivery fee and
            COD total before dispatch.
          </p>
        </motion.div>
      </aside>
    </div>
